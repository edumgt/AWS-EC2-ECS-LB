...
- 보안상 민감할 수 있는 AMI/키/호스트 정보는 마스킹 처리했습니다.

## Nginx 실행되는 VM 준비 (Ubuntu 기준)
아래 명령에서 root 접속의 경우 `sudo`를 생략합니다.
```bash
sudo apt update
...
```
Linux에서 `systemctl`은 systemd 서비스의 시작/중지, 상태 확인, 부팅 시 자동 실행 설정에 사용합니다.
...
## AMI 퍼블릭 공유
- ami-xxxxxxxx → Nginx 미설치 Ubuntu 버전
- ami-xxxxxxxx → Nginx 설치 버전
...
## SSH 접속 시도
```bash
ssh -i "<KEY_NAME>.pem" root@<EC2_HOSTNAME>
```
```
ssh: connect to host <EC2_HOSTNAME> port 22: Connection timed out
```
...
http://<EC2_HOSTNAME>/ 으로 접속 안될 수 있음.
...
An error occurred (AccessDenied) when calling the DescribeLoadBalancers operation: User: arn:aws:iam::<ACCOUNT_ID>:user/ErrorUser is not authorized to perform: elasticloadbalancing:DescribeLoadBalancers because no identity-based policy allows the elasticloadbalancing:DescribeLoadBalancers action
...
이미 IGW(igw-xxxxxxxx)는 VPC(vpc-xxxxxxxx)에 연결되어 있었음
→ AlreadyAssociated 오류로 확인

2. ALB 서브넷의 라우팅 테이블 확인
서브넷 ID: subnet-xxxxxxxx, subnet-xxxxxxxx
라우팅 테이블: rtb-xxxxxxxx → IGW로 나가는 경로가 없음
